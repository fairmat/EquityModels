...
\\
&=
DF(t,T) \mathbb{E}_t \left[ S_{T_0}  \tilde{C}^{BS}(\overline{K}, T-T_0, 1 ,\sigma) \right] 
\\ &=DF(t,T) S_0 e^{(r-q) (T_0-t)}   \tilde{C}^{BS}(\overline{K}, T-T_0, 1 ,\sigma) \ , 
\end{aligned}
$$
where $\tilde{C}^{BS}(\overline{K}, T-T_0, 1 , \sigma)$ is the undiscounted price of a call option under the Black-Scholes framework, with initial underlying value equal to $1$, volatility parameter $\sigma$, strike $\overline{K}$ and time to maturity $T-T_0$.
...
\\ 
&=
DF(t,T)  S_0 e^{(r-q) (T_0-t)}   \tilde{C}(\overline{K}, T-T_0, 1, \tilde{V}) \ , 
\end{aligned}
$$
with
\begin{equation}
...
\end{equation}
where  $\tilde{C}$ indicates the undiscounted call price under the Heston model. 
...
\\
&\approx
DF(t,T)   \tilde{C}(\overline{K}, T-T_0, 1, \tilde{V}) \ . 
\end{aligned}
$$
